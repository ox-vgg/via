/**
 * @class
 * @classdesc Manages region draw and view operations on an image or video (modified for scoliosis annotation)
...
_via_file_annotator.prototype._file_html_element_ready = function() {
  ...
  // guide container (i.e. for showing a visual guide to human annotators)
  this.guide_container = document.createElement('div');
  this.guide_container.setAttribute('id', 'guide_container');
  this.c.appendChild(this.guide_container);
  this._guide_init();
  this._guide_show();
...
_via_file_annotator.prototype._rinput_mouseup_handler = function(e) {
  ...
  if ( this.state_id === _VIA_RINPUT_STATE.REGION_DRAW_ONGOING ) {
    ...
    case _VIA_RSHAPE.SCOLIOSIS:
      this._state_set( _VIA_RINPUT_STATE.REGION_DRAW_NCLICK_ONGOING );
      _via_util_msg_show( 'Click on the 9 feature points (in order) as shown in guide.', true);
      break;
    default:
...
_via_file_annotator.prototype._draw = function(ctx, xy, is_selected) {
  ...
  case _VIA_RSHAPE.POLYLINE:
    ...
    break;
  case _VIA_RSHAPE.SCOLIOSIS:
    this._draw_scoliosis_region(ctx, xy, is_selected, shape_id);
    break;
  default:
    console.warn('_via_file_annotator._draw() : shape_id=' + shape_id + ' not implemented');
  }
...
_via_file_annotator.prototype._draw_scoliosis_region = function(ctx, pts, is_selected, shape_id) {
  ...
  if(n === 19) {
    ...
    var p9 = [ pts[17], pts[18] ];
    var v97 = [ p7[0] - p9[0], p7[1] - p9[1] ];
    var v98 = [ p8[0] - p9[0], p8[1] - p9[1] ];
    var dot_prod = v97[0]*v98[0] + v97[1]*v98[1];
    var mod97 = Math.sqrt(v97[0] * v97[0] + v97[1] * v97[1]);
    var mod98 = Math.sqrt(v98[0] * v98[0] + v98[1] * v98[1]);
    var cos_theta = dot_prod / (mod97 * mod98);
...
_via_file_annotator.prototype._draw_scoliosis_angle = function(ctx, angle, x, y) {
  ctx.font = '18px serif';
  ctx.fillStyle = 'white';
  ctx.fillText(Number.parseFloat(angle).toFixed(1) + '°', x + 20, y + 20);
}

_via_file_annotator.prototype._draw_scoliosis_project_pt_on_line = function(pts, x0, y0) {
  var x1 = pts[1];
  var y1 = pts[2];
  var x2 = pts[3];
  var y2 = pts[4];
  ...
  var m1 = (y2 - y1) / (x2 - x1);
  var y = ( m1*m1*y0 + y1 + m1*(x0 - x1) ) / (1 + m1*m1);
  var x = (y0 - y) * m1 + x0;
  return [x, y];
}

_via_file_annotator.prototype._draw_scoliosis_control_pt_label = function(ctx, id, x, y) {
  var dx = 0;
  var dy = 0;
  switch(id) {
  case 1:
  ...
  ctx.beginPath();
  ctx.arc(x + dx, y + dy, 14, 0, 2*Math.PI, false);
  ctx.closePath();
