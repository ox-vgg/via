...
_via_import_export.prototype.export_to_file = function(data_format) {
  ...
  case 'via3_csv':
    ...
    break;
  case 'scoliosis_project_csv':
    this.export_to_scoliosis_project_csv();
    break;
...
_via_import_export.prototype.export_to_scoliosis_project_csv = function() {
  return new Promise( function(ok_callback, err_callback) {
    var csv = [];

    this.d._cache_update();
    var csv_header = ['project_id','revision_id','file_id','filename','scoliosis_feature_points_xy'];
    for ( var aid in this.d.store.attribute ) {
      csv_header.push('"' + this.d.store.attribute[aid].aname + '"');
    }
    csv.push(csv_header.join(','));

    for ( var vindex in this.d.store.project.vid_list ) {
      var vid = this.d.store.project.vid_list[vindex];
      var line = [];
      line.push( this.d.store.project['pid'] );
      line.push( this.d.store.project['rev'] );
      ...
      for ( var mindex in this.d.cache.mid_list[vid] ) {
        ...
        if(this.d.store.metadata[mid]['xy'].length === 0) {
          ...
          var n = this.d.store.metadata[mid]['xy'].length;
          for(var i=1; i<n; ++i) {
            shape_metadata.push( this.d.store.metadata[mid].xy[i] );
          }
        }
      }
      line.push( '"' + JSON.stringify(shape_metadata) + '"');
      line.push( fmetadata.join(','));
      csv.push(line.join(','));
    }
    var data_blob = new Blob( [csv.join('\n')],
                              {type: 'text/csv;charset=utf-8'});
    var filename = [];
    filename.push(this.d.store.project.pid);
    filename.push(this.d.store.project.rev);
    filename.push('export.csv');
